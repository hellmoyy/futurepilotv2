...
export default function AdminLayout({
  ...
  const menuItems = [
    { href: '/administrator/dashboard', label: 'Dashboard', icon: 'home', section: 'main' },
    { href: '/administrator/users', label: 'Users', icon: 'users', section: 'main' },
    { href: '/administrator/referrals', label: 'Referrals', icon: 'referrals', section: 'main' },
    { href: '/administrator/commissions', label: 'Withdrawals', icon: 'card', section: 'main' },
    { href: '/administrator/trading-commissions', label: 'Trading Commissions', icon: 'money', section: 'main' },
    { href: '/administrator/transactions', label: 'Transactions', icon: 'list', section: 'main' },
    { href: '/administrator/custodial-wallet', label: 'Custodial Wallet', icon: 'wallet', section: 'main' },
  ...
  const getIcon = (iconName: string) => {
    const icons: any = {
      ...
      signal: <><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></>,
    ...
    <div className="min-h-screen bg-gray-900 flex">
      ...
      <aside className={`${sidebarOpen ? 'w-64' : 'w-20'} bg-gray-800 border-r border-gray-700 transition-all duration-300 flex flex-col fixed h-full z-10`}>
        ...
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
          ...
          {/* Bot Management Section */}
          {sidebarOpen && (
            <div className="pt-4 pb-2 mt-4 border-t border-gray-700">
              <p className="px-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">
                🤖 Bot Management
              </p>
            </div>
          )}
          {!sidebarOpen && <div className="pt-2 mt-2 border-t border-gray-700"></div>}
          
          {menuItems.filter(item => item.section === 'bot').map((item) => (
            <a
              key={item.href}
              href={item.href}
