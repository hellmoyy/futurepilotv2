  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="relative group">
        ...
        <div className="relative bg-white/[0.03] backdrop-blur-3xl rounded-3xl border border-white/10 p-6 hover:border-white/20 transition-all light:bg-gradient-to-br light:from-white light:to-blue-50 light:border-blue-200 light:hover:border-blue-300">
          ...
          <div className="relative">
            <h3 className="text-lg font-semibold text-white mb-4 light:text-gray-900">Your Referral Link</h3>
            ...
            <div className="mt-4 p-4 bg-blue-500/10 backdrop-blur-xl border border-blue-500/20 rounded-xl light:bg-blue-100 light:border-blue-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {Object.entries(membershipLevels).map(([key, level]) => (
          <div key={key} className={`relative bg-white/[0.03] backdrop-blur-3xl rounded-2xl border ${
            stats.membershipLevel === key ? 'border-white/30 light:border-blue-300' : 'border-white/10 light:border-blue-200'
          } p-6 light:bg-gradient-to-br light:from-white light:to-blue-50`}>
            <div className={`absolute inset-0 bg-gradient-to-br ${level.color} opacity-5 rounded-2xl light:opacity-10`}></div>
            <div className="relative text-center">
              <span className="text-3xl mb-2 block">{level.icon}</span>
              <h4 className="font-semibold text-white mb-1 light:text-gray-900">{level.name}</h4>
              <p className={`text-2xl font-bold bg-gradient-to-r ${level.color} bg-clip-text text-transparent`}>
                {level.rate}%
              </p>
              <p className="text-xs text-gray-500 mt-1 light:text-gray-600">Commission</p>
            </div>
          </div>
        ))}
      </div>
